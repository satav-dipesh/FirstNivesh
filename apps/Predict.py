...
def app():
    ...
    option = st.sidebar.text_input("Ticker","SEB")
    ...
    df3 = df1.tolist()
    st.write(f'Adjusted Closing for {option}')
    df4 = scaler.inverse_transform(df3)
    df4 = pd.DataFrame(df4,columns=['Adj Close'])
    st.line_chart(df4[1000:])
    df3.extend(lst_output)
    
    st.write(f'Prediction for the next {option2} days')
    df5 = scaler.inverse_transform(df3)
    df5 = pd.DataFrame(df5,columns=['Adj Close'])
    st.line_chart(df5[1000:])        
